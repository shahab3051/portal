...
const DATE_HEADERS_ = ['Date', 'D.O.J', 'Leave From', 'Leave To']; // ye columns hamesha date hone chahiye
...
/**
 * Agar koi date cell "Date" ki tarah type nahi hui (locale ya manual typing ki wajah se
 * Sheets ne usay plain text samjha), to yahan "1/9/2026" jaisi string ko khud detect kar
 * ke ISO "yyyy-MM-dd" mein badalte hain — Sheet ki locale dekh kar DD/MM ya MM/DD decide hota hai.
 */
function normalizeDateString_(s) {
  s = String(s).trim();
  if (/^\d{4}-\d{1,2}-\d{1,2}$/.test(s)) { // already ISO-ish
    const [y, m, d] = s.split('-');
    return y + '-' + String(m).padStart(2, '0') + '-' + String(d).padStart(2, '0');
  }
  const m = s.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})$/);
  if (!m) return s; // pehchan nahi payi, jaisi hai waisi rehne do
  ...
  if (a > 12) { day = a; month = b; }
    ...
    if (locale.indexOf('en_us') === 0 || locale === 'en-us') { month = a; day = b; }
    else { day = a; month = b; } // default: DD/MM (Pakistan aur zyada tar dusri locales)
  }
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    headers.forEach((h, idx) => {
      ...
      if (v instanceof Date) {
        // Google Sheets mein khaali "time" cell (e.g. Punch In "5:10 PM") bhi ek Date
        // object ke tor par aata hai, jiski date-part hamesha 1899-12-30 hoti hai —
        // usay date ki jagah "HH:mm:ss" time format mein rakho, warna time data corrupt ho jata hai.
        const isPureTime = v.getFullYear() === 1899;
        v = isPureTime
          ? Utilities.formatDate(v, Session.getScriptTimeZone(), 'HH:mm:ss')
          : Utilities.formatDate(v, Session.getScriptTimeZone(), 'yyyy-MM-dd');
      } else if (typeof v === 'string' && v !== '' && DATE_HEADERS_.indexOf(h) !== -1) {
        v = normalizeDateString_(v);
      }
      obj[h] = v;
    });
    rows.push(obj);
...
/** "HH:MM:SS" ya "HH:MM" → total minutes (null agar khali) */
/** "HH:MM:SS", "HH:MM" ya "H:MM AM/PM" — sab ko minutes mein convert karta hai (null agar khali/invalid) */
function timeToMinutes_(t) {
  if (t === '' || t === null || t === undefined) return null;
  const s = String(t).trim();
  let m = s.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?$/); // 24-hour "HH:mm[:ss]"
  if (m) return Number(m[1]) * 60 + Number(m[2]);
  m = s.match(/^(\d{1,2}):(\d{2})\s*([AaPp][Mm])$/); // 12-hour "h:mm AM/PM" (agar cell plain text ho)
  if (m) {
    let h = Number(m[1]) % 12;
...
/**
 * Business rule (jaisa aap ne bataya):
 * Punch In aa raha ho          → Present (Working Hours punch in/out se calculate, overnight shift bhi handle)
 * Punch In na ho + Weekend din → Weekend
 * Punch In na ho + weekday     → Absent
 */
function computeAttendanceStatus_(dateStr, punchIn, punchOut) {
  const inM = timeToMinutes_(punchIn);
  if (inM !== null) {
    let outM = timeToMinutes_(punchOut);
    let hours = 0;
    if (outM !== null) {
      if (outM < inM) outM += 24 * 60; // raat ko shift cross-midnight (e.g. 5 PM to 2 AM)
      hours = Math.round(((outM - inM) / 60) * 100) / 100;
    }
    return { status: 'Present', hours: hours };
  }
  return { status: isWeekend_(dateStr) ? 'Weekend' : 'Absent', hours: 0 };
}
/** Attendance sheet ki har row ke Working Hours/Status ko upar wale rule se dobara calculate karta hai */
function getComputedAttendanceAll_() {
  const rows = readSheet_('Attendance');
  return rows.map(r => {
    const calc = computeAttendanceStatus_(r['Date'], r['Punch In'], r['Punch Out']);
    r['Working Hours'] = calc.hours;
...
function getAttendance_(empId, month) {
  let rows = getComputedAttendanceAll_();
...
function punchAttendance_(p) {
  ...
  try {
    ...
    if (p.type === 'in' || rowIndex === -1) {
      sh.appendRow([p.date, p.empId, p.name, p.time, '', '', '']); // Working Hours/Status computed on read
    } else {
...
function getDashboard_(empId, role) {
  return getCached_('dashboard_' + role + '_' + empId, () => {
    const employees = readSheet_('Employee');
    const attendance = getComputedAttendanceAll_();
    ...
    if (role === 'Admin') {
      ...
      const onLeaveIds = new Set(onLeaveToday.map(r => String(r['EMP ID'])));

      // Har employee ke liye aaj ka status decide karo — agar row maujood hai to wahi
      // (Present/Weekend/Absent, upar wale rule se), warna khud isWeekend_ se nikalo.
      const byEmp = new Map(todayRows.map(r => [String(r['EMP ID']), r]));
      const todayFull = employees.map(e => {
        const empId2 = String(e['EMP ID']);
        if (onLeaveIds.has(empId2)) {
          return { 'Date': today, 'EMP ID': e['EMP ID'], 'Employee Name': e['Name'], 'Punch In': '', 'Punch Out': '', 'Working Hours': 0, 'Status': 'Leave' };
        }
        const row = byEmp.get(empId2);
        if (row) return row;
        return { 'Date': today, 'EMP ID': e['EMP ID'], 'Employee Name': e['Name'], 'Punch In': '', 'Punch Out': '', 'Working Hours': 0, 'Status': isWeekend_(today) ? 'Weekend' : 'Absent' };
      });

      return {
        totalEmployees: employees.length,
        presentToday: todayFull.filter(r => r['Status'] === 'Present').length,
        absentToday: todayFull.filter(r => r['Status'] === 'Absent').length,
        weekendToday: todayFull.filter(r => r['Status'] === 'Weekend').length,
        onLeaveToday: todayFull.filter(r => r['Status'] === 'Leave').length,
      ...
      return {
        ...
        weekendThisMonth: monthRows.filter(r => r['Status'] === 'Weekend').length,
